...
// ═══════════════════════════════════════════════════════════════
// Prediction Engine — clean rewrite
// ───────────────────────────────────────────────────────────────
// Single momentum-based predictor:
//   • EWMA realized vol (lambda=0.94) → per-minute σ on log returns
//   • Drift estimate from recent log returns (clamped)
//   • Lognormal probability of close-above-strike
//   • Signal pipeline: momentum / trend / rsi → confidence
//   • Bet quality via Kelly fraction over Kalshi YES ask
//   • Sell signal: only "lost_cause" or "confident_flip" trigger exits
// ───────────────────────────────────────────────────────────────
// All public functions referenced by server.js are preserved.
// ═══════════════════════════════════════════════════════════════

const store = require('./store');

// ── Tunables ─────────────────────────────────────────────────────
const EWMA_LAMBDA   = 0.94;     // RiskMetrics-style decay
const MIN_VOL       = 0.0002;   // floor on per-minute σ (log-return)
const MAX_VOL       = 0.02;     // ceiling
const RSI_PERIODS   = 14;
const TREND_LOOKBACK = 20;      // bars
...
function computeTrend(prices, lookback = TREND_LOOKBACK) {
    ...
    return slope / slice[slice.length - 1];
}
...
// ── Bet quality (Kelly over Kalshi YES ask) ──────────────────────
...
function assessBetQuality(prediction, strikePrice, marketData, minutesAhead) {
    ...
    const factors = {
        probability: probForBet,
        ask,
        edge,
        confidence: conf,
        choppiness,
        exhaustion,
        sessionMultiplier: riskMult,
        minutesAhead,
    };
...
function gradeBayesianPrediction(actualPrice, periodKey) {
    ...
    store.updateBayesianState((state) => {
        if (!state.calibrationBins) state.calibrationBins = {};
        const bin = Math.floor((entry.probability || 0.5) * 10) / 10;
        const key = bin.toFixed(1);
        if (!state.calibrationBins[key]) state.calibrationBins[key] = { wins: 0, total: 0 };
        state.calibrationBins[key].total += 1;
        if (correct) state.calibrationBins[key].wins += 1;
        return state;
    });
}
...
function gradePreviousPrediction(actualPrice, currentPeriodKey) {
    ...
    if (graded) {
        ...
        store.updateErrorAnalysis((ea) => {
            ...
            ea.records.push({
                periodKey: graded.periodKey,
                correct: graded.correct,
                probability: graded.probability,
                confidence: graded.confidence,
                predictedPrice: graded.predictedPrice,
                actualPrice: graded.actualPrice,
            });
...
function getErrorSummary() {
    const ea = store.getErrorAnalysis() || { records: [] };
    ...
    const bs = store.getBayesianState();
    return {
        total,
